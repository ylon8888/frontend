...
const StepNine = ({ data, isLoading }: { data: any; isLoading: boolean }) => {
  const [activeVideoUrl, setActiveVideoUrl] = useState<string | null>(null);
  const stepNineData = data?.data?.chapters?.[0]?.stepNine;

  // Function to play video
  const playVideo = () => setActiveVideoUrl(stepNineData?.stepVideo);
  const closeVideo = () => {
  ...
  return (
    <div className="flex flex-col space-y-6">
      ...
      <div className="bg-white rounded-lg p-8 border border-primary shadow-xl">
        {/* Content Sections */}
        <div
          dangerouslySetInnerHTML={{ __html: stepNineData?.stepDescription }}
        />
      </div>
