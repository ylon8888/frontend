      ...
      <div className="container max-w-[1320px] mx-auto px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-16 mx-auto">
          {stats.map((stat, index) => (
            <motion.div
              key={index}
              className="bg-[rgba(250,250,250,0.2)] p-8 rounded-2xl text-white text-center"
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ delay: stat.delay, duration: 0.6 }}
            >
              <h3 className="font-montserrat text-6xl font-semibold mb-8">
                {stat.number}
              </h3>
              <p className="text-3xl font-light">{stat.label}</p>
            </motion.div>
          ))}
        </div>
      </div>
